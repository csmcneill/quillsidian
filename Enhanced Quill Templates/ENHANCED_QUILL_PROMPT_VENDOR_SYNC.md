# Enhanced Vendor Sync Meeting Prompt

This prompt formats a vendor/external sync meeting (your organization + external participants). It gathers information related to a recorded meeting in the Quill macOS app and outputs markdown structured for use in Obsidian.
...
- Choose `project:`: `"stripe"` if any `@stripe.com` attendee, else `"merchant-of-record"`
...
`session_type`, `participants` (array), `external_partners` (array), `tags`, `source: "quill"`.
...
- `#compliance`, `#disputes`, `#merchant-of-record`, `#operationalization`, `#pricing`, `#quality-ops`, `#miscellaneous`
...
{
  "summary_markdown": "---\ndate: {{YYYY-MM-DD}}\nmeeting_title: \"Vendor Sync – {{Partner}}: {{Focus}}\"\nproject: \"{{stripe or merchant-of-record}}\"\nsession_type: \"external-sync\"\nexternal_partners: [\"{{Partner}}\", \"{{optional partner}}\"]\nparticipants: [\"{{Your Name}}\", \"{{Name}}\", \"{{Name}}\"]\nsummary: \"{{One-sentence outcome}}\"\nnext_meeting_focus: \"{{optional}}\"\ntags:\n  - \"#meeting\"\n  - \"#quill\"\n  - \"#vendor-sync\"\n  - \"#{{optional tag 1}}\"\n  - \"#{{optional tag 2}}\"\nsource: \"quill\"\n---\n# Vendor Sync – {{Focus}}\n\n## Summary\n{{1–2 short paragraphs on purpose and outcomes.}}\n\n## Key Topics Discussed\n- **{{Theme}}**: {{Details}}\n- **{{Theme}}**: {{Details}}\n\n## Followup Tasks\n- [ ] {{Task}}\n- [ ] {{Task}}\n\n## Open Questions or Follow-ups\n- {{Question / dependency}}\n\n## Detailed Notes\n{{Narrative-by-topic; multiple paragraphs for readability.}}\n\n## Systemic Insights (optional)\n{{Only if relevant.}}\n",
  "meeting_title": "Vendor Sync – {{Partner}}: {{Focus}}",
  "meeting_date": "{{YYYY-MM-DD}}",
  "quill_title": "{{optional: exact Meeting.title}}",
  "participants": ["{{Your Name}}", "{{Name}}", "{{Name}}"],
  "session_type": "external-sync",
  "external_partners": ["{{Partner}}"],
  "transcript_snippet": "{{First 500-1000 characters of transcript with <SNIP> if truncated}}"
}
